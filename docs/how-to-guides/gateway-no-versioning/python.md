...
Hosted-module versioning is separate from the pip package version on a generated Graft.

## Default behavior

- Standalone Gateway (no project key) **disables versioning by default**.
- `--noVersioning` forces versioning off.
- `--keepVersioning` can turn versioning on without a project key.
...
Reinstall consumers from Vision after contract changes.
