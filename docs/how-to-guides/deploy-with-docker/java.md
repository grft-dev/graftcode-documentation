...
Use a base image that includes the JVM runtime and install the Linux `gg` package from
...
CMD ["gg", "--runtime", "jvm", "--modules", "<module-path>"]
```

Host locally first with [Run Gateway locally](../run-gateway-locally.md), then containerize the same
`gg` command line.

**Gap:** no verified multi-stage Dockerfile for JVM is maintained here.
