...
to execute calls — see [Project Key, registry, host, and credentials](../reference/project-key-registry-host-and-credentials.md).
...
| `grft-test-simple-car-repository-e2e` | `0.2.1`–`0.2.2` (see [supported runtimes](../reference/supported-runtimes-and-package-managers.md)) |
...
[Configure invocation](configure-graft-invocation.md)—remote Grafts need a WebSocket host; some public
...
Install coordinates change when Gateway restarts without a [project key](use-a-portal-project-key.md).
...
(for example `http://localhost:81/GV`). See [Deploy with Docker](deploy-gateway-with-docker.md).
...
Never derive registry URLs from the Receiver assembly or module name. Use a [project key](use-a-portal-project-key.md)
