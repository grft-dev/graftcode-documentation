...
connections. Install and run it — see [Gateway and hosted modules](../core-concepts/gateway-and-hosted-modules.md).
...
3. **Host the module or install from the public repository.** To expose your own Receiver, run
   **Gateway** (`gg`): it loads the module, serves Vision, and publishes the model used to generate
   packages — see [Gateway and hosted modules](../core-concepts/gateway-and-hosted-modules.md). To consume a
   capability someone else published, install a Graft from the
   [public repository](../how-to-guides/obtain-and-install-a-graft.md) without
...
4. **Configuration selects monolith or microservice.** The same installed Graft can run
   **in-memory** in the Caller process (modular monolith) or **remotely** against a Gateway
   ...
   [Execution modes](../core-concepts/in-memory-same-machine-and-remote-execution.md), and
...
- [Choose a scenario](choose-your-scenario.md)
