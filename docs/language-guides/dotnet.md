...
**Provider: supported. Consumer: supported.** Graftcode provides .NET module analysis,
...
`InvokeWithHeaders(...)` are available in the generated package.
...
The Graftcode Engine explicitly rejects framework complex types in public APIs. Avoid
...
There is no universal supported-type matrix or guarantee for every target-framework moniker. Vision
and a generated package from your running Gateway are authoritative.
